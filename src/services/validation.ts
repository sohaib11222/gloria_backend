 ...
 */
export function validateUnlocode(unlocode: string): boolean {
...
export function validateVehicleClass(vehicleClass: string): boolean {
 ...
 */
export const unlocodeSchema = z.string().refine(
  (val) => validateUnlocode(val),
  { message: "UN/LOCODE must be 5 characters: 2-letter country code + 3-letter location code (e.g., GBMAN, USNYC)" }
);
...
export const isoDateSchema = z.string().refine(
  (val) => validateIsoDate(val),
  { message: "Date must be in ISO 8601 format (e.g., 2025-01-15T10:00:00Z)" }
);
 ...
 */
export const vehicleClassSchema = z.string().refine(
  (val) => validateVehicleClass(val),
  { message: `Vehicle class must be one of: ${VALID_VEHICLE_CLASSES.join(', ')}` }
);
