...
export const AvailabilityStore = {
	async createJob(params: CreateJobParams): Promise<string> {
		const expected =
			params.expectedSources ??
			(await prisma.agreement.count({
				where: {
					agentId: params.agentId,
					status: "ACTIVE",
					agreementRef: { in: params.agreementRefs },
				},
			}));
		...
		if (items.length === 0) {
			await prisma.availabilityResult.create({
				data: {
					...
					sourceId,
					offerJson: {
						error: timedOut ? "TIMEOUT" : "NO_RESULT",
						source_id: sourceId,
					},
				},
			});
	...
	async getJobSince(
		...
		const readOnce = async () => {
			...
			const status: JobStatus =
				job && job.status === "RUNNING" ? "IN_PROGRESS" : "COMPLETE";
	...
	async purgeExpiredJobs(ttlSeconds = 600): Promise<number> {
		...
		await prisma.availabilityResult.deleteMany({
			where: { jobId: { in: ids } },
		});
