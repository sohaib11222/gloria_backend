...
export function validateLocationPayload(
  ...
  if (!payload.Branchcode) missingFields.push("Branchcode");
  if (!payload.AtAirport) missingFields.push("AtAirport");
  if (!payload.LocationType) missingFields.push("LocationType");
  if (!payload.CollectionType) missingFields.push("CollectionType");
