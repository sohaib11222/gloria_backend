 ...
 * - Branchcode / BranchCode / Code (required for identification)
 * - Name
 * - Status, AgreementId (optional)
 ...
 * - LocationType, CollectionType, Brand
 * - NatoLocode / UNLocode / LocationCode
 * - Phone or Telephone, Email or EmailAddress
 * - PickupInstructions
 * - Monday..Sunday opening hours as "HH:mm - HH:mm"
 */
...
function normalizeHeader(h: string): string {
	...
	const map: Record<string, string> = {
		...
		brand: "brand",
		natolocode: "natolocode",
		nato_locode: "natolocode",
		unlocode: "natolocode",
		un_locode: "natolocode",
		locationcode: "natolocode",
		location_code: "natolocode",
		phone: "phone",
		telephone: "phone",
		phonenumber: "phone",
		email: "email",
	...
	return map[s] || map[compact] || compact || s;
}
...
function buildOpening(
	...
	for (const [key, label] of Object.entries(dayMap)) {
		...
		if (!value) continue;
		const [open, closed] = value.split(/\s*-\s*/);
		opening[label] = closed
			? { attr: { Open: open.trim(), Closed: closed.trim() } }
			: { attr: { Open: open.trim() } };
	}
	return Object.keys(opening).length ? opening : undefined;
}

function buildBranchFromRow(
	row: Record<string, string>,
	...
	const lat = row.latitude ? Number.parseFloat(row.latitude) : NaN;
	const lon = row.longitude ? Number.parseFloat(row.longitude) : NaN;
	...
	const opening = buildOpening(row);
	const pickupInstructions = row.pickupinstructions || "";
	...
	return {
		...
		agreementId: row.agreementid || undefined,
		AtAirport:
			atAirport === "true" || atAirport === "1" || atAirport === "yes"
				? "true"
				: atAirport === "false" || atAirport === "0" || atAirport === "no"
					? "false"
					: null,
		...
		Telephone: phone
			? { attr: { PhoneNumber: phone }, PhoneNumber: phone }
			: undefined,
		EmailAddress: email || undefined,
		email: email || undefined,
		Latitude: !Number.isNaN(lat) ? lat : undefined,
		Longitude: !Number.isNaN(lon) ? lon : undefined,
		NatoLocode: row.natolocode || undefined,
		natoLocode: row.natolocode || undefined,
		PickupInstructions: pickupInstructions
			? { attr: { Pickup: pickupInstructions } }
			: undefined,
		Opening: opening,
		Address: {
			...
			CountryName:
				countryCode || country
					? {
							...
							attr: { Code: (countryCode || "").toUpperCase().slice(0, 3) },
						}
					: undefined,
		},
...
export function parseCsvToBranches(
	...
	for (let r = 1; r < lines.length; r++) {
		...
		branches.push(buildBranchFromRow(row, defaultCountryCode));
	}
	return branches;
}
...
export async function parseExcelToBranches(
	...
	try {
		...
		if (e?.code === "MODULE_NOT_FOUND" || e?.message?.includes("xlsx")) {
			console.warn(
				"[branchCsvExcelParser] xlsx module not installed. Install with: npm install xlsx",
			);
			return [];
		}
		throw e;
