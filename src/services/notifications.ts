...
export async function notifyAgreementOffered(agreementId: string) {
  const ag = await prisma.agreement.findUnique({
    where: { id: agreementId },
    include: { agent: true, source: true }
  });
...
export async function notifyAgreementAccepted(agreementId: string) {
  const ag = await prisma.agreement.findUnique({
    where: { id: agreementId },
    include: { agent: true, source: true }
  });
...
export async function notifyAgreementStatus(agreementId: string, status: string) {
  const ag = await prisma.agreement.findUnique({
    where: { id: agreementId },
    include: { agent: true, source: true }
  });
