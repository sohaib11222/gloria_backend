...
export async function submitEcho(
  ...
  agentId: string,
  agreementRef: string,
  payload: { message: string; attrs: Record<string, string> }
): Promise<{
  requestId: string;
  ...
  const dispatchPromises = eligible.map(({ sourceId, agreementRef: agRef }) =>
    limit(async () => {
      ...
      try {
        ...
        try {
          ...
          let echoedAttrs: Record<string, string>;
 ...
 */
export async function getEchoResults(
  requestId: string,
  sinceSeq: bigint,
  waitMs: number
): Promise<{
  ...
  newItems: Array<{ echoedMessage: string; echoedAttrs: Record<string, string> }>;
  lastSeq: bigint;
  ...
  if (job.items.length > 0) {
    ...
    return {
      ...
      newItems: job.items.map((item) => ({
        ...
        echoedAttrs: (item.echoedAttrs as Record<string, string>) || {},
      })),
      lastSeq: job.lastSeq,
...
function generateEtag(requestId: string, lastSeq: string): string {
