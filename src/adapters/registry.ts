...
class MockAdapter implements SourceAdapter {
	...
	async availability(c: AvailabilityCriteria): Promise<Offer[]> {
		...
		const mk = (n: number) => ({
			...
			currency: "USD",
			total_price:
				Math.round((19 + base + n * 7 + (c.driver_age - 21)) * 100) / 100,
...
export async function getAdapterForSource(
	sourceId: string,
	...
	if (!src) {
		console.error(
			`[AdapterRegistry] ❌ Source ${sourceId} not found in database`,
		);
	...
	console.log(`[AdapterRegistry] 📊 Source company found:`, {
		...
		availabilityEndpointUrl: src.availabilityEndpointUrl,
	...
	if (
		typeof (src as any).api_base_url === "string" &&
		(src as any).api_base_url.startsWith("grpc://")
	) {
		grpcAddr = (src as any).api_base_url.replace("grpc://", "");
		console.log(
			`[AdapterRegistry] 📍 Found grpc:// in api_base_url: ${grpcAddr}`,
		);
	}
	...
	if (!grpcAddr && src.grpcEndpoint && src.grpcEndpoint.startsWith("grpc://")) {
		...
		console.log(
			`[AdapterRegistry] 📍 Found grpc:// prefix in grpcEndpoint, extracted: ${grpcAddr}`,
		);
	}
	...
	if (
		!grpcAddr &&
		src.grpcEndpoint &&
		!src.grpcEndpoint.startsWith("http://") &&
		!src.grpcEndpoint.startsWith("https://")
	) {
		grpcAddr = src.grpcEndpoint;
		console.log(
			`[AdapterRegistry] 📍 grpcEndpoint is host:port format (no protocol), assuming gRPC: ${grpcAddr}`,
		);
	} else if (
		src.grpcEndpoint &&
		(src.grpcEndpoint.startsWith("http://") ||
		...
		console.log(
			`[AdapterRegistry] ⚠️ grpcEndpoint has http/https protocol: ${src.grpcEndpoint}`,
		);
	}
	...
	if (useGrpc || grpcAddr) {
		const addr =
			grpcAddr ||
			`${(src as any).grpc_host || "localhost"}:${(src as any).grpc_port || 50061}`;
		console.log(
			`[AdapterRegistry] ✅ Selected: gRPC adapter (real gRPC protocol) for source ${sourceId} at ${addr}`,
		);
		console.log(`[AdapterRegistry] 🔧 Creating GrpcSourceAdapter instance...`);
		return makeGrpcSourceAdapter(addr) as any;
	}

	// HTTP availability endpoint configured from the Source pricing tab. This may be a full
	// endpoint URL (for example https://supplier.test/pricetest.php), so pass it as direct availability URL.
	if (src.availabilityEndpointUrl && src.availabilityEndpointUrl.trim()) {
		const GrpcAdapter = await loadGrpcAdapter();
		const endpoint =
			src.httpEndpoint?.trim() || new URL(src.availabilityEndpointUrl).origin;
		console.log(
			`[AdapterRegistry] ✅ Selected: HTTP availability adapter for source ${sourceId} at ${src.availabilityEndpointUrl}`,
		);
		return new GrpcAdapter({
			endpoint,
			availabilityEndpointUrl: src.availabilityEndpointUrl.trim(),
			authHeader: process.env.SUPPLIER_GRPC_AUTH || "",
			sourceId,
			useOtaAvailabilityXml: true,
			availabilityRequestorId:
				src.locationListAccountId || src.companyCode || "1000097",
		} as any);
	}
	...
	if (
		...
		const GrpcAdapter = await loadGrpcAdapter();
		console.log(
			`[AdapterRegistry] ✅ Selected: HTTP adapter (GrpcAdapter) for source ${sourceId} at ${src.grpcEndpoint}`,
		);
		return new GrpcAdapter({
			endpoint: src.grpcEndpoint,
			authHeader: process.env.SUPPLIER_GRPC_AUTH || "",
			sourceId,
		});
	}
	...
	if (src.adapterType === "mock") {
		...
		const allowMockInProduction =
			process.env.ALLOW_MOCK_ADAPTER_IN_PRODUCTION === "true";

		if (isProduction && !allowMockInProduction) {
			console.error(
				`[AdapterRegistry] ❌ BLOCKED: Mock adapter cannot be used in production for source ${sourceId}`,
			);
			throw new Error("MOCK_ADAPTER_NOT_ALLOWED_IN_PRODUCTION");
		}

		console.warn(
			`[AdapterRegistry] ⚠️ WARNING: Using MOCK adapter for source ${sourceId} (TEST-ONLY)`,
		);
		console.warn(`[AdapterRegistry] ⚠️ Source: ${src.companyName} (${src.id})`);
		console.warn(
			`[AdapterRegistry] ⚠️ This adapter returns fake data and should only be used for testing`,
		);
	...
	if (
		...
		!grpcAddr &&
		!src.grpcEndpoint &&
		!src.availabilityEndpointUrl
	) {
		console.error(
			`[AdapterRegistry] ❌ ERROR: Source ${sourceId} has no adapter configured`,
		);
		console.error(
			`[AdapterRegistry] ❌ Source must have adapterType set to "grpc" or "http", or configure grpcEndpoint`,
		);
	...
	console.warn(
		`[AdapterRegistry] ⚠️ WARNING: Falling back to Mock adapter for source ${sourceId} - this should not happen`,
	);
	console.warn(
		`[AdapterRegistry] ⚠️ Source configuration: adapterType=${src.adapterType}, grpcEndpoint=${src.grpcEndpoint}`,
	);
	return new MockAdapter();
}
 ...
 */
export async function isSourceUsingMockAdapter(
	sourceId: string,
): Promise<boolean> {
	try {
		...
		console.error(
			`[AdapterRegistry] Error checking mock adapter for source ${sourceId}:`,
			error,
		);
		return false;
	}
