...
import {
	buildOtaVehResRQ,
	buildOtaVehAvailRateRQ,
	convertToOtaBookingData,
} from "../services/otaXmlBuilder.js";
import type { Offer } from "./registry.js";

/**
 * Detect if response is OTA VehAvailRS-shaped (VehAvailRSCore with VehVendorAvails).
 */
export function isOtaVehAvailResponse(response: any): boolean {
 ...
 */
export function parseOtaVehAvailResponse(
	response: any,
	sourceId: string,
	criteria: any,
): Offer[] {
	...
	const pickup_location_details =
		pickupAttrs.LocationCode || pickupAttrs.Locationname
			? {
			...
			: undefined;
	const return_location_details =
		returnAttrs.LocationCode || returnAttrs.Locationname
			? {
	...
	for (const vva of vendorAvailList) {
		...
		for (let i = 0; i < vehAvailList.length; i++) {
			...
			const included = asArray(vehTerms.Included).map((x: any) => {
			...
			const notIncluded = asArray(vehTerms.NotIncluded).map((x: any) => {
				const inner = Array.isArray(x) ? x[0] : x;
				const t = inner?.["@attributes"];
				return t
					? {
			...
			const rentalRate =
				(Array.isArray(coreNode.RentalRate)
					? coreNode.RentalRate[0]
					: coreNode.RentalRate) ?? {};
			...
			const vehicleChargeList = asArray(vehicleCharges.VehicleCharge).map(
				(ch: any) => {
					const a = attrs(ch);
					const out: any = {
						Amount: a.Amount,
			...
			const total_charge =
				totalChargeAttrs.RateTotalAmount != null ||
				totalChargeAttrs.CurrencyCode
					? {
			...
			const calcRaw =
				calculationNode &&
				typeof calculationNode === "object" &&
				calculationNode["@attributes"]
					? calculationNode["@attributes"]
					: calculationNode;
			const calcAttrs = calcRaw && typeof calcRaw === "object" ? calcRaw : {};
			const calculation =
				calcAttrs.UnitCharge || calcAttrs.UnitName
					? {
			...
			const priced_equips: any[] = [];
			for (const pe of pricedEquipsRaw) {
				const list = Array.isArray(pe)
					? pe
					: pe && pe.PricedEquip != null
						? asArray(pe.PricedEquip)
						: [pe];
				for (const p of list) {
					...
					const calcA =
						calc && typeof calc === "object" && calc["@attributes"]
							? calc["@attributes"]
							: typeof calc === "object"
								? calc
								: {};
					priced_equips.push({
						...
						vendor_equip_id:
							equipAttrs.vendorEquipID ?? equipAttrs.vendorEquipId,
						charge: {
			...
			const name = makeModelAttrs.Name ?? (vehicle as any).Name;
			const totalPriceNum =
				total_charge?.rate_total_amount != null
					? parseFloat(String(total_charge.rate_total_amount))
					: NaN;
			const supplier_offer_ref =
				vehID ||
				(criteria.agreement_ref && name
					? `GEN-${(criteria.agreement_ref || "").substring(0, 10)}-${(sourceId || "").substring(0, 8)}-${i}-${Buffer.from(
							`${criteria.agreement_ref}-${name}-${totalPriceNum}-${i}`,
						)
							.toString("base64")
							.substring(0, 8)
							.replace(/[^A-Za-z0-9]/g, "")}`
			...
			const offer: Offer = {
				...
				supplier_offer_ref:
					supplier_offer_ref || `GEN-${sourceId}-${i}-${Date.now()}`,
				...
				vehicle_charges: vehicleChargeList.length
					? vehicleChargeList
					: undefined,
...
export class GrpcAdapter {
	constructor(
		private config: {
			...
			authHeader: string;
			sourceId: string;
			useOtaXml?: boolean; // Optional flag to use OTA XML format for booking
			useOtaAvailabilityXml?: boolean; // When true, send OTA_VehAvailRateRQ XML for availability
			availabilityEndpointUrl?: string; // Optional full availability URL from Source pricing tab
			availabilityRequestorId?: string; // RequestorID for OTA availability (default 1000097)
	...
	private async makeRequest(
		...
		if (endpoint.startsWith("grpc://")) {
			throw new Error(
				`GrpcAdapter (HTTP-based) cannot handle grpc:// endpoint. Use GrpcSourceAdapter instead. Endpoint: ${endpoint}`,
			);
		}
		...
		if (!endpoint.startsWith("http://") && !endpoint.startsWith("https://")) {
			...
			console.log(
				`[GrpcAdapter] Added http:// protocol to endpoint: ${endpoint}`,
			);
		}
		...
		try {
			...
			console.log(
				`[GrpcAdapter] Response status: ${response.status} ${response.statusText}`,
			);

			if (!response.ok) {
				const errorText = await response.text().catch(() => "Unknown error");
				console.error(
					`[GrpcAdapter] HTTP error ${response.status}:`,
					errorText,
				);
	...
	async locations(): Promise<string[]> {
		try {
			...
			return response
				.map((loc: any) => loc.LocationCode || loc.unlocode || loc)
				.filter(Boolean);
		} catch (error) {
	...
	async availability(criteria: any): Promise<any[]> {
		try {
			...
			const path =
				this.config.availabilityEndpointUrl?.trim() || "/availability";
			const url =
				path.startsWith("http://") || path.startsWith("https://")
					? path
			...
			console.log(`[GrpcAdapter] Making availability request to: ${url}`, {
				...
				endpoint: this.config.endpoint,
				directAvailabilityUrl: this.config.availabilityEndpointUrl || null,
			});
			...
			if (typeof response === "string") {
				...
				if (
					...
					try {
						...
						response = parsed;
						console.log(
							`[GrpcAdapter] Parsed availability response as JSON (was string)`,
						);
					} catch {
			...
			if (typeof response === "object" && isOtaVehAvailResponse(response)) {
				const offers = parseOtaVehAvailResponse(
					response,
					this.config.sourceId,
					criteria,
				);
				console.log(
					`[GrpcAdapter] Parsed OTA VehAvailRS: ${offers.length} offers for source ${this.config.sourceId}`,
				);
				return offers;
			}
			...
			if (!Array.isArray(response)) {
				console.warn(
					`[GrpcAdapter] Expected array or OTA response, got:`,
					typeof response,
				);
				return [];
			}
			...
			const offers = response.map((offer: any, index: number) => {
				// Generate supplier_offer_ref if missing
				let supplier_offer_ref =
					offer.SupplierOfferRef || offer.supplier_offer_ref || "";
				if (!supplier_offer_ref) {
					...
					const agreementRefShort = (criteria.agreement_ref || "").substring(
						0,
						10,
					);
					const offerHash = Buffer.from(
						`${criteria.agreement_ref}-${offer.VehicleClass || offer.vehicle_class || ""}-${offer.VehicleMakeModel || offer.vehicle_make_model || ""}-${offer.TotalPrice || offer.total_price || 0}-${index}`,
					)
						.toString("base64")
						.substring(0, 8)
						.replace(/[^A-Za-z0-9]/g, "");
					...
					console.log(
						`[GrpcAdapter] 🔧 Generated supplier_offer_ref for offer ${index}:`,
						{
							original:
				...
				return {
					...
					vehicle_make_model:
						offer.VehicleMakeModel || offer.vehicle_make_model,
					...
					availability_status:
						offer.AvailabilityStatus || offer.availability_status,
				};
			});

			console.log(
				`[GrpcAdapter] Converted ${offers.length} offers for source ${this.config.sourceId}`,
			);
			return offers;
		} catch (error) {
			const errorMessage =
			...
			const errorStack = error instanceof Error ? error.stack : undefined;
			console.error(
				`[GrpcAdapter] Failed to fetch availability for source ${this.config.sourceId}:`,
				{
					error: errorMessage,
					stack: errorStack,
					endpoint: this.config.endpoint,
	...
	async bookingCreate(input: any): Promise<any> {
		try {
			...
			const hasFullBookingData = !!(
				input.pickup_unlocode ||
				input.pickup_iso ||
				input.vehicle_class ||
				input.driver_age
			);
			const useOtaXml =
				this.config.useOtaXml && hasFullBookingData && this.config.agentId;
			...
			if (useOtaXml) {
				...
				const agentCompanyName =
					input.agent_company_name || this.config.agentCompanyName;
			...
			return {
				supplier_booking_ref:
					response.SupplierBookingRef || response.supplier_booking_ref,
				status: response.Status || response.status || "REQUESTED",
				agreement_ref:
					response.AgreementRef ||
					response.agreement_ref ||
					input.agreement_ref,
				supplier_offer_ref:
					response.SupplierOfferRef || response.supplier_offer_ref,
			};
		} catch (error) {
	...
	async bookingCheck(ref: string, agreement_ref: string): Promise<any> {
		try {
			...
			const response = await this.makeRequest(
				"GET",
				`/booking/check/${ref}?agreement_ref=${encodeURIComponent(agreement_ref)}`,
			);
