 ...
 */
export function parseOtaVehAvailResponse(response: any, sourceId: string, criteria: any): Offer[] {
  const core = response.VehAvailRSCore;
  if (!core) return [];
  ...
  for (const vva of vendorAvailList) {
    ...
    for (let i = 0; i < vehAvailList.length; i++) {
      ...
      const vehicleChargeList = asArray(vehicleCharges.VehicleCharge).map((ch: any) => {
        const a = attrs(ch);
        const out: any = { Amount: a.Amount, CurrencyCode: a.CurrencyCode, TaxInclusive: a.TaxInclusive, GuaranteedInd: a.GuaranteedInd, Purpose: a.Purpose };
