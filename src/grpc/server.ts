...
import {
	...
	buildCheckBookingResponse,
} from "../services/otaResponseBuilder.js";
import {
	adapterLatency,
...
function normalizeBookingStatus(value: any, fallback = "REQUESTED") {
...
async function isLocationAllowedForSource(sourceId: string, unlocode: string) {
	const code = String(unlocode || "").trim();
	if (!code) return false;
	const variants = [...new Set([code, code.toUpperCase(), code.toLowerCase()])];

	const base = await prisma.sourceLocation.findFirst({
		where: { sourceId, unlocode: { in: variants } },
		select: { id: true },
	});
	if (base) return true;
	...
	const branch = await prisma.branch.findFirst({
		where: {
			sourceId,
			OR: [{ natoLocode: { in: variants } }, { branchCode: { in: variants } }],
		},
		select: { id: true },
	});
...
export async function startGrpcServers() {
	...
	server.addService(AvailabilityService, {
		Submit: async (call: any, cb: any) => {
			...
			try {
				...
				// 2) Resolve explicit agreement search or marketplace-style source discovery.
				const directSourceSearch =
					!Array.isArray(c.agreement_refs) || c.agreement_refs.length === 0;
				if (directSourceSearch) c.agreement_refs = [];

				console.log(
					`[Availability.Submit] 🔍 Resolving availability targets:`,
					{
						agent_id,
						directSourceSearch,
						status: "ACTIVE",
				...
				let agreements: any[] = [];
				if (directSourceSearch) {
					const configuredSources = await prisma.company.findMany({
						where: {
							type: "SOURCE",
							status: "ACTIVE",
							OR: [
								{ availabilityEndpointUrl: { not: null } },
								{ grpcEndpoint: { not: null } },
								{ httpEndpoint: { not: null } },
								{ adapterType: "mock" },
							],
						},
						select: {
					...
					const usableSources = configuredSources.filter((src) => {
						const hasConfiguredEndpoint = Boolean(
							src.availabilityEndpointUrl?.trim() ||
								src.grpcEndpoint?.trim() ||
								src.httpEndpoint?.trim() ||
								src.adapterType === "mock",
						);
						return hasConfiguredEndpoint;
					});

					agreements = usableSources.map((src) => ({
						id: `direct-${src.id}`,
						agreementRef: `DIRECT-${src.companyCode || src.id.slice(0, 8)}`,
						sourceId: src.id,
						directSourceSearch: true,
						sourceName: src.companyName,
					}));
				} else {
				...
				console.log(`[Availability.Submit] 📊 Targets found:`, {
					...
					agreements: agreements.map((a: any) => ({
						...
						sourceId: a.sourceId,
						directSourceSearch: !!a.directSourceSearch,
					})),
				});
				...
				console.log(
					`[Availability.Submit] 📈 Expected sources: ${expected_sources} (unique source IDs)`,
				);

				// 3) Always create a job (even if 0 sources), so we can return a request_id
				...
				logger.debug(
					{
						jobId,
						expected_sources,
						refs: c.agreement_refs,
						directSourceSearch,
					},
					"[Availability.Submit]",
				);

				// 4) If no target sources, finish early with a valid request_id
				if (expected_sources === 0) {
					console.log(
						`[Availability.Submit] ⚠️ No sources found, returning early:`,
						{
							jobId,
							agent_id,
							agreementRefs: c.agreement_refs,
							directSourceSearch,
							targetsFound: agreements.length,
						},
					);
					return cb(null, {
						request_id: jobId,
				...
				// 5) Coverage check + health/subscription filtering.
				console.log(
					`[Availability.Submit] 🔍 Checking ${agreements.length} targets for location coverage and health...`,
				);
				const eligible: { ag: any }[] = [];
				for (const ag of agreements) {
					const okPick = ag.directSourceSearch
						? await isLocationAllowedForSource(ag.sourceId, c.pickup_unlocode)
						: await isLocationAllowedForAgreement(ag.id, c.pickup_unlocode);
					const okDrop = ag.directSourceSearch
						? await isLocationAllowedForSource(ag.sourceId, c.dropoff_unlocode)
						: await isLocationAllowedForAgreement(ag.id, c.dropoff_unlocode);
					...
					console.log(
						`[Availability.Submit] 📍 Target ${ag.agreementRef} (source: ${ag.sourceId}):`,
						{
							pickup: c.pickup_unlocode,
							dropoff: c.dropoff_unlocode,
							okPick,
							...
							isExcluded,
							hasSubscription,
							directSourceSearch: !!ag.directSourceSearch,
				...
				console.log(
					`[Availability.Submit] ✅ Eligible targets after filtering: ${eligible.length} out of ${agreements.length}`,
				);

				if (eligible.length === 0) {
					console.log(
						`[Availability.Submit] ⚠️ No eligible targets after location/health checks, marking job complete`,
					);
				...
				console.log(
					`[Availability.Submit] 🚀 Starting parallel fan-out to ${eligible.length} eligible agreements`,
				);
				...
				const runOne = async (ag: any) => {
					console.log(
						`[Availability.Submit] 🎬 runOne() called for agreement ${ag.agreementRef} (source: ${ag.sourceId})`,
					);
					const startTime = Date.now();
					console.log(
						`[Availability.Submit] 🔧 Getting adapter for source ${ag.sourceId} (agreement: ${ag.agreementRef})...`,
					);

					const adapter = await getAdapterForSource(ag.sourceId);

					console.log(
						`[Availability.Submit] ✅ Adapter obtained for source ${ag.sourceId}:`,
						{
							adapterType: adapter?.constructor?.name || "unknown",
							hasAvailabilityMethod:
								typeof adapter?.availability === "function",
					...
					console.log(
						`[Availability.Submit] ⏱️ Timeout set to ${TIMEOUT_MS}ms for source ${ag.sourceId}`,
					);

					try {
						console.log(
							`[Availability.Submit] 📞 Calling adapter.availability() for source ${ag.sourceId}:`,
							{
						...
						console.error(
							`[Availability.Submit] ❌ Adapter error for source ${ag.sourceId} (${latency}ms):`,
							{
				...
				try {
					console.log(
						`[Availability.Submit] 📋 About to execute Promise.allSettled for ${eligible.length} agreements`,
					);
					const promises = eligible.map(({ ag }, index) => {
						console.log(
							`[Availability.Submit] 📝 Creating promise ${index + 1}/${eligible.length} for agreement ${ag.agreementRef} (source: ${ag.sourceId})`,
						);
						return limit(() => {
							console.log(
								`[Availability.Submit] 🔄 limit() callback executing for agreement ${ag.agreementRef}`,
							);
							return runOne(ag);
						});
					});
					console.log(
						`[Availability.Submit] ⏳ Waiting for ${promises.length} promises to complete...`,
					);
					const results = await Promise.allSettled(promises);
					console.log(
						`[Availability.Submit] ✅ All promises settled:`,
						results.map((r, i) => ({
							index: i,
							status: r.status,
		...
		Poll: async (call: any, cb: any) => {
			...
			try {
				...
				const out = await AvailabilityStore.getJobSince(
					String(request_id),
					Number(since_seq),
					Number(wait_ms),
				);
	...
	server.addService(BookingService, {
			...
			try {
				console.log("[Booking.Create] 📥 Received gRPC booking request:", {
					...
					idempotency_key: req?.idempotency_key
						? `${req.idempotency_key.substring(0, 20)}...`
						: "MISSING",
				...
				if (!idempotency_key) {
					console.error(
						"[Booking.Create] ❌ Missing idempotency_key in gRPC request:",
						{
				...
				if (!agreement_ref)
					return cb({ code: 3, message: "agreement_ref is required" });

				if (isDirectAgreementRef(agreement_ref)) {
					const source = await prisma.company.findFirst({
						where: { id: source_id, type: "SOURCE", status: "ACTIVE" },
						select: { id: true },
					});
					if (!source)
						return cb({ code: 9, message: "SOURCE_INACTIVE or not found" });
				} else {
				...
				if (customer_info_json) {
					try {
						customerInfo =
							typeof customer_info_json === "string"
				...
				if (payment_info_json) {
					try {
						paymentInfo =
							typeof payment_info_json === "string"
				...
				const createStatus = normalizeBookingStatus(
					supplierResp.status,
					"REQUESTED",
				);
				...
				const { recordBookingCreated } = await import(
					"../services/bookingHistory.js"
				);
				...
				const otaResponse = await buildBookingResponse(
					bookingData,
					supplierResp,
				);
			...
			try {
				...
				if (!agentId) {
					...
					if (!booking) {
						return cb({
							code: 5,
							message: "Booking not found for this agreement",
						});
					}
				...
				const modifyStatus = normalizeBookingStatus(resp.status, "CONFIRMED");

				// Update booking with modification data if provided
				const updateData: any = {
					status: modifyStatus,
					payloadJson: resp as any,
				};
				...
				if (booking && agentId) {
					const { recordBookingModified } = await import(
						"../services/bookingHistory.js"
					);
					const modifiedFields = Object.keys(updateData).filter(
						(k) => k !== "status" && k !== "payloadJson",
					);
					await recordBookingModified(
						bookingBefore,
						booking,
						agentId,
						modifiedFields,
		...
		Cancel: async (call: any, cb: any) => {
			...
			try {
				...
				// Validate formal agreements; direct Source bookings are authorized by the existing booking row.
				if (!isDirectAgreementRef(agreementRef)) {
					const ag = await prisma.agreement.findFirst({
						where: {
							agentId: agentId,
							sourceId: source_id,
				...
				const adapter = await getAdapterForSource(source_id);
				const resp = await adapter.bookingCancel(
					supplier_booking_ref,
					agreementRef,
				);
				const cancelStatus = normalizeBookingStatus(resp.status, "CANCELLED");
				await prisma.booking.updateMany({
					where: {
						supplierBookingRef: supplier_booking_ref,
						sourceId: source_id,
					},
					data: { status: cancelStatus as any, payloadJson: resp as any },
				});

				// Record booking cancellation in history
				const { recordBookingCancelled } = await import(
					"../services/bookingHistory.js"
				);
		...
		Check: async (call: any, cb: any) => {
			...
			try {
				...
				const adapter = await getAdapterForSource(source_id);
				const resp = await adapter.bookingCheck(
					supplier_booking_ref,
					agreementRef,
				);
				...
				const otaResponse = await buildCheckBookingResponse(
					bookingDataArray,
					isSingleMatch,
				);
	...
	server.addService(AgreementService, {
		CreateDraft: async (call: any, cb: any) => {
			try {
				const { agent_id, source_id, agreement_ref, valid_from, valid_to } =
					call.request;
				...
				try {
					...
					console.log(
						`🔍 gRPC Debug - Agent result: ${JSON.stringify(agentRow)}`,
					);
					...
					console.log(
						`🔍 gRPC Debug - Source result: ${JSON.stringify(sourceRow)}`,
					);
				} catch (error) {
		...
		ListBySource: async (call: any, cb: any) => {
			try {
				...
				logger.error(
					{ error: e, source_id: sourceIdParam },
					"ListBySource error",
				);
	...
	server.addService(VerificationService, {
			...
			try {
				...
				if (!source_id || source_id === "MOCK-SOURCE-ID") {
					return cb({
						...
						message:
							"source_id is required and cannot be MOCK-SOURCE-ID. Provide a real source company ID.",
					});
				}
		...
		GetVerificationStatus: async (call: any, cb: any) => {
			try {
				...
				const result =
					await VerificationRunner.getVerificationStatus(company_id);
		...
		SubmitEcho: async (call: any, cb: any) => {
			try {
				...
				if (!agent_id || !agreement_ref) {
					return cb({
						code: 3,
						message: "agent_id and agreement_ref required",
					});
				}
	...
	server.bindAsync(
		...
		(err, port) => {
			if (err) {
				logger.error(
					{ error: err.message, port: CORE_PORT },
					"gRPC core server bind error (non-fatal)",
				);
				return;
			}
			server.start();
