...
import {
	...
	isHttpsMailApiConfigured,
} from "../../infra/mailer.js";
...
type ParsedRegistrationPhoto = { buf: Buffer; ext: string };

function parseRegistrationPhotoDataUrl(
	dataUrl: string,
): ParsedRegistrationPhoto | { error: string } {
	const m = dataUrl
		.trim()
...
async function persistRegistrationPhoto(
	companyId: string,
	parsed: ParsedRegistrationPhoto,
): Promise<string> {
	const dir = path.join(process.cwd(), "uploads", "registration-photos");
	await fs.mkdir(dir, { recursive: true });
	const filename = `${companyId}-${Date.now()}.${parsed.ext}`;
	await fs.writeFile(path.join(dir, filename), parsed.buf);
	return `/api/uploads/registration-photos/${filename}`;
}
	...
	async (req, res, next) => {
		try {
			const limit = Math.max(
				1,
				Math.min(200, Number((req.query?.limit as string) || 50)),
			);
	...
	async (_req, res, next) => {
		try {
			...
			for (const s of sources) {
				const count = await prisma.sourceLocation.count({
					where: { sourceId: s.id },
				});
				out.push({
					sourceId: s.id,
	...
	async (req, res, next) => {
		try {
			const schema = z.object({
				...
				owner_type: z.enum(["agent", "source", "admin"]).optional(),
			...
			const out = await createApiKey({
				name: p.name || "API Key",
				ownerType: p.owner_type || "admin",
				ownerId: p.owner_id || "default",
	...
	async (req, res, next) => {
		try {
			...
			if (!existing) {
				return res
					.status(404)
					.json({ error: "API_KEY_NOT_FOUND", message: "API key not found" });
			}
...
adminRouter.get(
	...
	async (_req, res, next) => {
		try {
			const items = await prisma.whitelistedIp.findMany({
				orderBy: { createdAt: "desc" },
			});
			res.json({ items });
		} catch (e) {
...
adminRouter.post(
	...
	async (req, res, next) => {
		try {
			...
			const p = schema.parse(req.body);
			const item = await prisma.whitelistedIp.upsert({
				where: { ip_type: { ip: p.ip, type: p.type } as any },
				update: { enabled: p.enabled },
				create: { ip: p.ip, type: p.type, enabled: p.enabled } as any,
			});
		...
		try {
			...
			if (!existing) {
				return res
					.status(404)
					.json({
						error: "WHITELIST_ENTRY_NOT_FOUND",
...
adminRouter.get(
	...
	async (_req, res, next) => {
		try {
			const [agents, sources, agreements, activeKeys, last24hRequests] =
				await Promise.all([
					prisma.company.count({ where: { type: "AGENT" } }),
					prisma.company.count({ where: { type: "SOURCE" } }),
					prisma.agreement.count({
						where: {
							status: { in: ["ACTIVE", "DRAFT", "OFFERED", "ACCEPTED"] },
						},
					}),
		...
		try {
			...
			const items = rows.map((s) => {
				...
				return {
					...
					pickupLoc:
						s.pickupLoc ??
						(criteria?.pickupLoc != null ? String(criteria.pickupLoc) : ""),
					returnLoc:
						s.returnLoc ??
						(criteria?.returnLoc != null ? String(criteria.returnLoc) : ""),
					pickupIso:
						s.pickupIso ??
						(criteria?.pickupIso != null ? String(criteria.pickupIso) : ""),
					returnIso:
						s.returnIso ??
						(criteria?.returnIso != null ? String(criteria.returnIso) : ""),
...
adminRouter.get(
	...
	async (_req, res, next) => {
		try {
			...
			const stats = new Map<
				string,
				{ total: number; AGENT: number; SOURCE: number }
			>();
		...
		try {
			...
			if (slug.length < 2) {
				return res
					...
					.json({
						...
						message: "Slug is too short after normalization.",
					});
			}
			...
			if (e?.code === "P2002") {
				return res
					...
					.json({
						error: "CONFLICT",
						message: "A referral link with this slug already exists.",
					});
			}
			next(e);
		}
		...
		try {
			...
			const updated = await prisma.referralLink.update({
				...
				data: {
					...
					...(body.restrictToType !== undefined
						? { restrictToType: body.restrictToType }
						: {}),
				},
			});
			res.json(updated);
			...
			if (e?.code === "P2025") {
				return res
					.status(404)
					.json({ error: "NOT_FOUND", message: "Referral link not found." });
			}
			next(e);
		}
	},
...
adminRouter.get(
	"/admin/referral-links/:id/signups",
	requireAuth(),
	requireRole("ADMIN"),
	async (req, res, next) => {
		try {
	...
	async (req, res, next) => {
		try {
			...
			const updatedCompany = await prisma.company.update({
				where: { id },
				data: {
					status: status as "ACTIVE" | "PENDING_VERIFICATION" | "SUSPENDED",
				},
				include: {
					users: true,
					agentAgreements: true,
			...
			if (sendEmail) {
				...
				if (company.email?.trim())
					recipients.add(company.email.trim().toLowerCase());
...
<p style="margin-top:20px;font-size:14px;color:#6b7280;">If you have questions, contact your administrator or Gloria Connect support.</p>
					...
					try {
						...
						if (failed.length === 0) {
							...
							emailSent = true;
							emailError =
								`Sent to ${results.length - failed.length} of ${results.length} address(es). ${failed[0]?.reason?.message || ""}`.trim();
						}
					} catch (mailErr: any) {
						emailSent = false;
...
adminRouter.post(
	...
	async (req, res, next) => {
		try {
			...
			if (adapterType) {
				if (!["mock", "grpc", "http"].includes(adapterType)) {
					return res.status(400).json({
						...
						message:
							"Adapter type must be one of: grpc, http" +
							(isProduction ? "" : " (mock allowed in development only)"),
					});
				}
				...
				if (isProduction && adapterType === "mock") {
					return res.status(400).json({
						...
						message:
							"Mock adapter type is not allowed in production. Please use 'grpc' or 'http'.",
					});
				}
			}
			...
			if (
				status &&
				!["ACTIVE", "PENDING_VERIFICATION", "SUSPENDED"].includes(status)
			) {
				return res.status(400).json({
					error: "INVALID_STATUS",
					message:
						"Status must be one of: ACTIVE, PENDING_VERIFICATION, SUSPENDED",
				});
			}

			if (registrationPhotoDataUrl && type !== "SOURCE") {
				return res.status(400).json({
					error: "INVALID_INPUT",
					message:
						"Registration photos are only accepted for supplier (SOURCE) accounts.",
				});
			}

			let parsedRegistrationPhoto: ParsedRegistrationPhoto | null = null;
			const photoRaw =
				typeof registrationPhotoDataUrl === "string"
					? registrationPhotoDataUrl.trim()
					: "";
			if (photoRaw) {
				const parsed = parseRegistrationPhotoDataUrl(photoRaw);
				if ("error" in parsed) {
					return res
						.status(400)
						.json({ error: "INVALID_PHOTO", message: parsed.error });
				}
				parsedRegistrationPhoto = parsed;
			}
				...
				if (u) {
					try {
						...
						return res
							...
							.json({
								error: "INVALID_URL",
								message: "companyWebsiteUrl must be a valid URL",
							});
					}
				}
			}
			...
			let newCompany = await prisma.company.create({
				data: {
					...
					grpcEndpoint: grpcEndpoint ? String(grpcEndpoint).trim() : null,
					httpEndpoint: httpEndpoint ? String(httpEndpoint).trim() : null,
					companyCode: companyCode ? String(companyCode).trim() : null,
					billingCountryCode: billingCountryCode
						? String(billingCountryCode).toUpperCase().slice(0, 2)
						: null,
					registrationBranchName: registrationBranchName
						? String(registrationBranchName).trim()
						: null,
			...
			if (parsedRegistrationPhoto) {
				try {
					const registrationPhotoUrl = await persistRegistrationPhoto(
						newCompany.id,
						parsedRegistrationPhoto,
					);
					...
					console.error(
						"admin company registration photo persist failed",
						photoErr,
					);
				}
			}
	...
	async (req, res, next) => {
		try {
			...
			if (billingCountryCode !== undefined)
				updateData.billingCountryCode = billingCountryCode
					? String(billingCountryCode).toUpperCase().slice(0, 2)
					: null;
			if (httpEndpoint !== undefined)
				updateData.httpEndpoint = httpEndpoint
					? String(httpEndpoint).trim()
					: null;
			...
			if (registrationBranchName !== undefined) {
				updateData.registrationBranchName = registrationBranchName
					? String(registrationBranchName).trim()
					: null;
			}
			...
			if (removeRegistrationPhoto === true) {
				updateData.registrationPhotoUrl = null;
			}
			const photoRaw =
				typeof registrationPhotoDataUrl === "string"
					? registrationPhotoDataUrl.trim()
					: "";
			if (photoRaw) {
				const targetType = type || existingCompany.type;
				...
				updateData.registrationPhotoUrl = await persistRegistrationPhoto(
					id,
					parsed,
				);
			}
...
adminRouter.get(
	...
	async (req, res, next) => {
		try {
			...
			const sourceIds = [
				...new Set(
					rows.map((r) => r.sourceId).filter((id): id is string => Boolean(id)),
				),
			];
			...
			const items = rows.slice(0, p.limit).map((item) => {
				...
				try {
					parsedRequest = item.maskedRequest
						? JSON.parse(item.maskedRequest)
						: null;
				} catch (e) {
				...
				try {
					parsedResponse = item.maskedResponse
						? JSON.parse(item.maskedResponse)
						: null;
				} catch (e) {
			...
			const getAction = (endpoint: string | null): string => {
				...
				if (parts.length >= 2) {
					...
					return (
						action.charAt(0).toUpperCase() + action.slice(1) + " " + parts[0]
					);
				}
				return endpoint
					.replace(/\./g, " ")
			...
			const getResource = (item: any): string => {
				...
				if (item.endpoint) {
					...
					if (item.endpoint.includes("availability"))
			...
			const activityItems = items.map((item) => {
				...
				if (item.httpStatus && item.httpStatus >= 400) {
					details = details
						? `${details}, HTTP ${item.httpStatus}`
						: `HTTP ${item.httpStatus}`;
				}
				if (item.grpcStatus && item.grpcStatus !== 0) {
					details = details
						? `${details}, gRPC ${item.grpcStatus}`
						: `gRPC ${item.grpcStatus}`;
				}
		...
		try {
			const [companies, agreements, bookings, jobs24h, errors24h] =
				await Promise.all([
					...
					prisma.auditLog.count({
						where: {
							createdAt: { gte: new Date(Date.now() - 24 * 3600 * 1000) },
							OR: [{ httpStatus: { gte: 400 } }, { grpcStatus: { gt: 0 } }],
						},
					}),
				]);
			...
			res.json({
				companiesByStatus: Object.fromEntries(
					companies.map((r) => [r.status, r._count._all]),
				),
				agreementsByStatus: Object.fromEntries(
					agreements.map((r) => [r.status, r._count._all]),
				),
				bookingsByStatus: Object.fromEntries(
					bookings.map((r) => [r.status, r._count._all]),
				),
	...
	async (_req, res, next) => {
		try {
			...
			const { checkAgentHealth } = await import(
				"../../grpc/clients/agent-grpc.client.js"
			);
			const { checkSourceHealth } = await import(
				"../../grpc/clients/source-grpc.client.js"
			);
			...
			const lastSync =
				sourcesWithSync.length > 0 && sourcesWithSync[0].lastLocationSyncAt
...
adminRouter.get(
	"/admin/sources/health",
	requireAuth(),
	requireCompanyType("ADMIN", "AGENT"),
	async (req, res, next) => {
		try {
			const sources = await prisma.company.findMany({
	...
	async (req: any, res, next) => {
		try {
			...
			await auditLog({
				...
				request: { sourceId },
				response: {
					message: "Source health reset successfully",
					sourceId,
...
adminRouter.post(
	...
	async (req: any, res, next) => {
		try {
			...
			await Promise.all(
				sources.map((source) =>
					SourceHealthService.resetSourceHealth(source.id, adminId),
				),
			);
		...
		try {
			...
			if (
				...
				return res.status(400).json({
					error: "SCHEMA_ERROR",
					message:
						"Invalid agent_id or source_id - companies must exist and have correct types",
				});
			}
	...
	async (req, res, next) => {
		try {
			...
			if (source.type !== "SOURCE") {
				return res
					.status(400)
					.json({ error: "INVALID_TYPE", message: "Company is not a SOURCE" });
			}
			...
			const httpEndpoint = source.httpEndpoint || `http://localhost:9090`;
			...
			try {
				...
				for (const branch of branches) {
					const branchData = {
						...
						latitude:
							typeof branch.Latitude === "number" ? branch.Latitude : null,
						longitude:
							typeof branch.Longitude === "number" ? branch.Longitude : null,
	...
	async (req, res, next) => {
		try {
			...
			const bySourceWithNames = bySource.map((item) => ({
				sourceId: item.sourceId,
				sourceName:
					sources.find((s) => s.id === item.sourceId)?.companyName || "Unknown",
				count: item._count,
			}));
				...
				if (!raw) {
					...
					try {
						data.natoLocode = await ensureUnlocodeRowForBranch(raw, {
							countryCode: (body.countryCode ?? existing.countryCode) as
								| string
								| null,
		...
		try {
			...
			res.json({
				items: unlocodes.map((loc) => ({
					...
					usageCount:
						loc._count.sourceLocations + loc._count.agreementLocationOverrides,
				})),
				total,
				limit,
				offset,
...
adminRouter.patch(
	...
	async (req, res, next) => {
		try {
			...
			if (body.iataCode !== undefined)
				updateData.iataCode = body.iataCode?.toUpperCase() || null;
	...
	async (req, res, next) => {
		try {
			...
			for (let i = 0; i < lines.length; i++) {
				const line = lines[i];
				const [unlocode, country, place, iataCode, lat, lon] = line
					.split(",")
					.map((s) => s.trim());

				if (!unlocode || !country || !place) {
					errors.push(
						`Line ${i + 1}: Missing required fields (unlocode, country, place)`,
					);
					continue;
				}
...
adminRouter.get(
	...
	async (req, res, next) => {
		try {
			...
			if (
				operation &&
				["create", "modify", "cancel", "check"].includes(operation)
			) {
			...
			const sourceIds = [
				...new Set(
					logs.map((r) => r.sourceId).filter((id): id is string => Boolean(id)),
				),
			];
			const allCompanyIds = [...new Set([...companyIds, ...sourceIds])];
...
adminRouter.get(
	...
	async (req, res, next) => {
		try {
			...
			const readMarkerIds = new Set(
				readMarkers.map((m) => m.id.replace("read-marker-", "")),
			);
		...
		try {
			...
			if (id.startsWith("cl")) {
				...
				try {
					...
					console.warn(
						`Could not create read marker for notification ${id}:`,
						error,
					);
				}
			}
...
const smtpConfigSchema = z.object({
	...
	fromEmail: z
		...
		.default("no-reply@carhire.local"),
			...
			if (existing) {
				...
				if (!body.password || body.password === "KEEP_EXISTING") {
					return res.status(400).json({
						error: "VALIDATION_ERROR",
						message:
							"Password is required when creating a new SMTP configuration",
					});
				}
		...
		try {
			...
			if (!isConfigured) {
				return res.status(400).json({
					...
					message:
						"Mail is not configured. Use admin SMTP, EMAIL_* in .env, or BREVO_API_KEY / SENDGRID_API_KEY / RESEND_API_KEY.",
