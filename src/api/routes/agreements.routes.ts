 ...
 */
agreementsRouter.get("/agent/companies", requireAuth(), requireCompanyType("AGENT"), async (req: any, res, next) => {
  try {
    ...
    const branchCountMap = new Map(branchCounts.map((b: any) => [b.sourceId, b._count]));
    const locationCountMap = new Map(locationCounts.map((l: any) => [l.sourceId, l._count]));

    const agreementsBySource = agreements.reduce<Record<string, any[]>>((acc, ag) => {
...
agreementsRouter.post("/agent/source-groups", requireAuth(), requireCompanyType("AGENT"), async (req: any, res, next) => {
  try {
    const agentId = req.user.companyId as string;
...
agreementsRouter.patch("/agent/source-groups/:id", requireAuth(), requireCompanyType("AGENT"), async (req: any, res, next) => {
  try {
    const agentId = req.user.companyId as string;
...
agreementsRouter.delete("/agent/source-groups/:id", requireAuth(), requireCompanyType("AGENT"), async (req: any, res, next) => {
  try {
    const agentId = req.user.companyId as string;
    const id = String(req.params.id || "");
    const sourceGroupDelegate = prismaAny.agentSourceGroup;
...
agreementsRouter.post("/agent/source-groups/:id/agreements", requireAuth(), requireCompanyType("AGENT"), async (req: any, res, next) => {
  try {
    const agentId = req.user.companyId as string;
...
agreementsRouter.delete("/agent/source-groups/:id/agreements/:agreementId", requireAuth(), requireCompanyType("AGENT"), async (req: any, res, next) => {
  try {
    const agentId = req.user.companyId as string;
    const groupId = String(req.params.id || "");
