...
authRouter.get("/auth/referral/:slug", async (req, res, next) => {
	...
	try {
		...
		if (!slug || slug.length < 2) {
			return res
				.status(400)
				.json({ error: "INVALID_SLUG", message: "Invalid referral code." });
		}
...
authRouter.post("/auth/register", async (req, res, next) => {
	...
	try {
		...
		try {
			const otp = await EmailVerificationService.sendOTPEmail(
				body.email,
				body.companyName,
			);
			emailSent = true;
			console.log(
				`✅ Registration successful for ${body.email}, OTP sent: ${otp}`,
			);
		} catch (emailErr: any) {
			emailSent = false;
			...
			console.error(
				`❌ Registration succeeded but failed to send OTP email to ${body.email}:`,
				emailErr,
			);
		...
		const response: any = {
			...
			...(emailIssue && {
				emailIssue: {
					...
					...(emailIssue.externalApiAttempted === true &&
					emailIssue.externalApiSummary
		...
		if (!emailSent && process.env.NODE_ENV !== "production") {
			...
			if (company?.emailOtp) {
				console.log(`\n${"=".repeat(80)}`);
				console.log(
					`🔑 REGISTRATION OTP FOR ${body.email}: ${company.emailOtp}`,
				);
				console.log(
					`⚠️  Email sending failed - OTP shown in console for development only`,
				);
...
authRouter.post("/auth/verify-email", async (req, res, next) => {
	...
	try {
		...
		const isValid = await EmailVerificationService.verifyOTP(
			body.email,
			body.otp,
		);
...
authRouter.post("/auth/resend-otp", async (req, res, next) => {
	try {
		...
		if (!isSmtpConfigured) {
			...
			errorDetails =
				"Mail is not configured. No admin SMTP, no EMAIL_* SMTP env vars, and no BREVO_API_KEY / SENDGRID_API_KEY / RESEND_API_KEY.";
		} else if (emailError) {
			if (
				emailError.includes("Authentication") ||
				emailError.includes("BadCredentials") ||
				emailError.includes("Username and Password not accepted")
			) {
				errorType = "auth_failed";
				errorDetails =
					"SMTP credentials are incorrect. For Gmail, use an App Password (not your regular password).";
		...
		const response: any = {
			...
			help: !emailSent
				? {
						...
						configureViaEnv:
							"Set EMAIL_HOST, EMAIL_USER, EMAIL_PASS in .env file (no quotes around values)",
						...
						gmailHelp:
							"For Gmail: Enable 2-Step Verification and use App Password from https://myaccount.google.com/apppasswords",
						checkConsole:
							"In development mode, check server console for OTP and detailed error logs",
						note:
		...
		if (!emailSent && otp) {
			...
			if (isDev) {
				...
				console.error(
					`❌ Email sending failed for ${email}. OTP was generated but not sent.`,
				);
				console.error(
					`   Please check SMTP configuration in admin panel or environment variables.`,
				);
			}
		}
 ...
 */
// Handle OPTIONS preflight for login route - COMPLETELY OPEN
// NOTE: Global middleware already handles OPTIONS, but this ensures it works
authRouter.options("/auth/login", (req, res) => {
	// CRITICAL: Check if headers already set to avoid duplicates
		...
		try {
			...
			if (user.company.approvalStatus !== "APPROVED") {
				return sendError(403, {
					error: "NOT_APPROVED",
					message:
						user.company.approvalStatus === "PENDING"
			...
			const userData = {
				...
				company: {
					...
					status: user.company.status,
					approvalStatus: user.company.approvalStatus,
					companyWebsiteUrl: user.company.companyWebsiteUrl || null,
					registrationBranchName: user.company.registrationBranchName || null,
					companyAddress: user.company.companyAddress || null,
					registrationPhotoUrl: user.company.registrationPhotoUrl || null,
			...
			try {
				...
				console.error(
					"Database error in login:",
					JSON.stringify(errorInfo, null, 2),
				);
			} catch (logError) {
				console.error(
					"Database error in login (logging failed):",
					String(dbError),
				);
			}
...
authRouter.post("/auth/verify-reset-otp", async (req, res, next) => {
	try {
		...
		const isValid = await PasswordResetService.verifyResetOTP(
			body.email,
			body.otp,
		);
