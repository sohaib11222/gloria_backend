...
function toAddr(input: string) {
  // allow "grpc://host:port" or "host:port"
  return input.startsWith("grpc://") ? input.replace("grpc://", "") : input;
}

export const testSourceGrpc = async (req: any, res: any) => {
  try {
    const { address, availabilityPayload, runBooking = false, bookingPayload } = req.body || {};
    if (!address) return res.status(400).json({ message: "address required (grpc://host:port or host:port)" });
...
export const testAgentGrpc = async (req: any, res: any) => {
  try {
