...
function authed(call) {
    const token = getBearer(call.metadata);
    if (!token)
        throw { code: grpc.status.UNAUTHENTICATED, message: "missing bearer token" };
    try {
...
async function normalizeCriteria(agentCompanyId, raw) {
    const c = { ...(raw || {}) };
...
export async function startPublicGrpcServer() {
    const pkg = loadPkg();
    const Service = pkg.agent.AgentIngressService.service;
    const server = new grpc.Server();
    server.addService(Service, {
        // ✅ Fixed: SubmitAvailability
        SubmitAvailability: async (call, cb) => {
            ...
            try {
                ...
                client.Submit({ agent_id: u.companyId, criteria }, (err, resp) => {
                    if (err)
                        return cb(err);
                    ...
                    const reqIdFromDownstream = (resp && (resp.request_id ?? resp.requestId ?? resp.rid)) || '';
                    const expectedSources = (resp && (resp.expected_sources ?? resp.expectedSources)) ?? 0;
                    const recommendedPollMs = (resp && (resp.recommended_poll_ms ?? resp.recommendedPollMs)) ?? 1500;
        ...
        CreateBooking: (call, cb) => {
            try {
                ...
                }, (err, resp) => (err ? cb(err) : cb(null, resp)));
            }
            catch (e) {
                cb(e);
            }
        },
        ModifyBooking: async (call, cb) => {
            try {
                const u = authed(call);
                const r = call.request;
        ...
        ListAgreements: (call, cb) => {
            try {
                ...
                prisma.agreement
                    ...
                    .then((rows) => cb(null, { agreement_ids: rows.map((r) => r.id) }))
                    .catch((err) => cb(err));
            }
            catch (e) {
                cb(e);
