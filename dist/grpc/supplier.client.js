...
export function makeSupplierClient(endpoint) {
    const pkg = loadPkg();
    const Client = pkg.supplier.SupplierService;
    const client = new Client(endpoint, grpc.credentials.createInsecure());
    return client;
}
export function metaWithAuth(apiKeyOrJwt) {
    const md = new grpc.Metadata();
    if (apiKeyOrJwt)
        md.set("authorization", String(apiKeyOrJwt)); // e.g., 'Bearer <token>' or 'ApiKey <key>'
    return md;
}
