...
export function buildApp() {
    ...
    app.post("/api/sources/webhook/stripe", express.raw({ type: "application/json", limit: "10mb" }), (req, res) => handleStripeWebhook(req, res));
    app.post("/sources/webhook/stripe", express.raw({ type: "application/json", limit: "10mb" }), (req, res) => handleStripeWebhook(req, res));
    ...
    const metricsCors = (res) => {
        res.setHeader('Access-Control-Allow-Origin', '*');
        res.setHeader('Access-Control-Allow-Methods', 'GET, OPTIONS');
        res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization');
    };
    const servePrometheusText = async (_req, res) => {
        try {
            metricsCors(res);
    ...
    const servePrometheusJson = async (_req, res) => {
        try {
            metricsCors(res);
    ...
    app.options('/metrics', (_req, res) => {
        metricsCors(res);
        res.status(204).end();
    });
    app.get('/api/metrics', servePrometheusText);
    app.options('/api/metrics', (_req, res) => {
        metricsCors(res);
        res.status(204).end();
    });
    app.get('/metrics/json', servePrometheusJson);
    app.options('/metrics/json', (_req, res) => {
        metricsCors(res);
        res.status(204).end();
    });
    app.get('/api/metrics/json', servePrometheusJson);
    app.options('/api/metrics/json', (_req, res) => {
        metricsCors(res);
        res.status(204).end();
    });
    mountSwagger(app);
