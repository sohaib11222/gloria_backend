 ...
 */
endpointsRouter.get("/endpoints/config", requireAuth(), async (req, res, next) => {
    try {
 ...
 */
endpointsRouter.put("/endpoints/config", requireAuth(), async (req, res, next) => {
    try {
        ...
        if (body.httpEndpoint) {
            try {
                ...
                return res.status(400).json({
                    ...
                    message: "HTTP endpoint must be a valid URL (e.g., 'http://localhost:9091')",
                });
            }
        }
        ...
        if (body.locationEndpointUrl) {
            try {
                ...
                return res.status(400).json({
                    ...
                    message: "Location endpoint URL must be a valid URL (e.g., 'https://example.com/locations.php')",
                });
            }
        }
        ...
        res.json({
            ...
            httpEndpoint: updatedCompany.httpEndpoint ||
 ...
 */
endpointsRouter.post("/endpoints/test", requireAuth(), async (req, res, next) => {
    try {
 ...
 */
endpointsRouter.get("/endpoints/status", requireAuth(), async (req, res, next) => {
    try {
        const company = await prisma.company.findUnique({
            where: { id: req.user.companyId },
            select: {
...
endpointsRouter.post("/endpoints/reset", requireAuth(), async (req, res, next) => {
    try {
        const company = await prisma.company.findUnique({
            where: { id: req.user.companyId },
        ...
        // Use ports that don't conflict with main API (8080) and gRPC servers (50051, 50052)
        const defaultHttpEndpoint = company.type === "AGENT"
            ? "http://localhost:9091"
            : "http://localhost:9090";
        res.json({
 ...
 *       - bearerAuth: []
 */
endpointsRouter.get("/settings", requireAuth(), async (req, res, next) => {
    try {
        const company = await prisma.company.findUnique({
            where: { id: req.user.companyId },
            select: {
 ...
 */
endpointsRouter.post("/settings/whitelist", requireAuth(), async (req, res, next) => {
    try {
 ...
 */
endpointsRouter.get("/endpoints/setup-status", requireAuth(), async (req, res, next) => {
    try {
        const company = await prisma.company.findUnique({
            where: { id: req.user.companyId },
            select: {
