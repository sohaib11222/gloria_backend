...
adminRouter.get("/admin/whitelist", requireAuth(), requireRole("ADMIN"), async (_req, res, next) => {
    try {
        const items = await prisma.whitelistedIp.findMany({ orderBy: { createdAt: 'desc' } });
        res.json({ items });
    }
    catch (e) {
        next(e);
...
adminRouter.post("/admin/whitelist", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        ...
        const row = await prisma.whitelistedIp.upsert({
            where: { ip_type: { ip: body.ip.trim(), type: body.type } },
...
adminRouter.post("/admin/ip-whitelist", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        ...
        const p = schema.parse(req.body);
        const item = await prisma.whitelistedIp.upsert({ where: { ip_type: { ip: p.ip, type: p.type } }, update: { enabled: p.enabled }, create: { ip: p.ip, type: p.type, enabled: p.enabled } });
...
adminRouter.get("/admin/referral-links", requireAuth(), requireRole("ADMIN"), async (_req, res, next) => {
    try {
        ...
        for (const row of grouped) {
            const id = row.referralLinkId;
            if (!stats.has(id))
                stats.set(id, { total: 0, AGENT: 0, SOURCE: 0 });
...
adminRouter.put("/admin/companies/:id", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        ...
        if (adapterType)
            updateData.adapterType = adapterType;
...
adminRouter.get("/admin/logs", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        ...
        if (p.fromIso || p.toIso) {
            where.createdAt = {};
            if (p.fromIso)
                where.createdAt.gte = new Date(p.fromIso);
            if (p.toIso)
                where.createdAt.lte = new Date(p.toIso);
        }
        if (p.q) {
        ...
        const companyIds = [...new Set(rows.map(r => r.companyId).filter((id) => Boolean(id)))];
        const sourceIds = [...new Set(rows.map(r => r.sourceId).filter((id) => Boolean(id)))];
        ...
        const getActor = (endpoint, companyType, sourceType) => {
            if (!endpoint)
                return 'system';
        ...
        const getResource = (item) => {
            if (item.agreementRef)
                return `Agreement ${item.agreementRef}`;
            if (item.requestId)
                return `Request ${item.requestId.slice(0, 16)}...`;
            if (item.companyName)
                return item.companyName;
            if (item.sourceName)
                return item.sourceName;
            if (item.endpoint) {
                if (item.endpoint.includes('booking'))
                    return 'Booking operation';
                if (item.endpoint.includes('availability'))
                    return 'Availability request';
                if (item.endpoint.includes('agreement'))
                    return 'Agreement';
                if (item.endpoint.includes('health'))
                    return 'Health check';
                if (item.endpoint.includes('location'))
                    return 'Location sync';
            }
            return 'System resource';
        };
        // Helper function to determine result
        const getResult = (httpStatus, grpcStatus) => {
            if (httpStatus) {
                if (httpStatus >= 400)
                    return 'error';
                if (httpStatus >= 300)
 ...
 */
adminRouter.post("/admin/health/check/:companyId", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        const startTime = Date.now();
        const requestId = req.requestId;
...
adminRouter.post("/admin/health/reset", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        ...
        await Promise.all(sources.map(source => SourceHealthService.resetSourceHealth(source.id, adminId)));
        const duration = Date.now() - startTime;
 ...
 *       - bearerAuth: []
 */
adminRouter.post("/admin/agreements", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
...
adminRouter.get("/admin/booking-logs", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        ...
        // Fetch company names for better display
        const companyIds = [...new Set(logs.map(r => r.companyId).filter((id) => Boolean(id)))];
        const sourceIds = [...new Set(logs.map(r => r.sourceId).filter((id) => Boolean(id)))];
        const allCompanyIds = [...new Set([...companyIds, ...sourceIds])];
        const companies = await prisma.company.findMany({
            where: { id: { in: allCompanyIds } },
            select: { id: true, companyName: true, type: true, companyCode: true }
 ...
 */
adminRouter.get("/admin/smtp", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
 ...
 *                 type: boolean
 */
adminRouter.post("/admin/smtp", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
 ...
 *       - bearerAuth: []
 */
adminRouter.patch("/admin/smtp", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
 ...
 *       - bearerAuth: []
 */
adminRouter.delete("/admin/smtp", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        const existing = await prisma.smtpConfig.findFirst({
            orderBy: { updatedAt: 'desc' },
        });
