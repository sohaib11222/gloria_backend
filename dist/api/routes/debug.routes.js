...
async function getPm2Logs(lines = 50) {
    try {
        // Get PM2 process info using pm2 jlist
        const { stdout: pm2List } = await execAsync("pm2 jlist");
        const processes = JSON.parse(pm2List);
        const backendProcess = processes.find((p) => p.name === "gloria-backend");
        if (!backendProcess) {
...
async function getPm2ErrorLogs(lines = 50) {
    try {
        // Get PM2 process info using pm2 jlist
        const { stdout: pm2List } = await execAsync("pm2 jlist");
        const processes = JSON.parse(pm2List);
...
debugRouter.get("/api/debug", async (req, res) => {
...
debugRouter.options("/api/debug", (req, res) => {
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Methods', '*');
    res.setHeader('Access-Control-Allow-Headers', '*');
    res.setHeader('Access-Control-Allow-Credentials', 'false');
...
debugRouter.get("/debug", async (req, res) => {
...
debugRouter.options("/debug", (req, res) => {
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Methods', '*');
    res.setHeader('Access-Control-Allow-Headers', '*');
    res.setHeader('Access-Control-Allow-Credentials', 'false');
