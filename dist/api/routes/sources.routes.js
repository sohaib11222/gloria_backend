...
async function checkBranchQuota(sourceId, subscribedBranchCount, addingBranches, addingLocations) {
...
sourcesRouter.post("/sources/import-branches", requireAuth(), requireCompanyType("SOURCE"), async (req, res, next) => {
    try {
        ...
        try {
            ...
            const branchFormat = source.branchEndpointFormat?.toUpperCase?.() || null;
...
sourcesRouter.post("/sources/upload-branches", requireAuth(), requireCompanyType("SOURCE"), async (req, res, next) => {
    try {
        ...
        if (subscriptionCheckUpload)
            return res.status(subscriptionCheckUpload.status).json(subscriptionCheckUpload.body);
        // Get companyCode from authenticated user's company (automatically from source)
        // companyCode is optional - if not set, validation will skip CompanyCode checks
        const companyCode = source.companyCode || undefined;
        const data = req.body;
        ...
        const uploadFormat = data?.format?.toUpperCase?.() || null;
        const uploadDefaultCountry = data?.defaultCountryCode?.trim?.() || source.branchDefaultCountryCode?.trim?.() || null;
        let branches = [];
        ...
        if (data && typeof data === 'object' && data.rawContent && typeof data.rawContent === 'string') {
            const rawValue = data.rawContent.trim();
        ...
        let content = null;
        if (branches.length === 0 && data && typeof data === 'object' && data.rawContent && typeof data.rawContent === 'string') {
        ...
        if (subUpload?.plan) {
            ...
            if (limitCheckUpload)
                return res.status(limitCheckUpload.status).json(limitCheckUpload.body);
        }
...
sourcesRouter.post("/sources/import-locations", requireAuth(), requireCompanyType("SOURCE"), async (req, res, next) => {
    try {
        ...
        if (subscriptionCheckLoc)
            return res.status(subscriptionCheckLoc.status).json(subscriptionCheckLoc.body);
    ...
    try {
        ...
        if (isXmlResponse && responseText.includes(responseRoot)) {
            try {
                ...
                function regexExtractLocations(xmlText) {
                    ...
                    while ((ldMatch = locationDetailRegex.exec(xmlText)) !== null) {
                        ...
                        if (addressMatch) {
                            ...
                            if (cnMatch)
                                address.CityName = { value: cnMatch[1].trim() };
                        ...
                        for (const day of days) {
                            ...
                            if (dayMatch) {
                                ...
                                if (openMatch)
                                    opening[day] = { attr: { Open: openMatch[1].replace(/\n/g, "").trim() } };
                            }
                        }
                        const normalized = {
                            attr: attrs,
                ...
                let xmlParserError = null;
                let diagnostics = { responseLength: responseText.length, responseExcerpt: responseText.substring(0, 200) };
                try {
                    const { XMLParser } = await import("fast-xml-parser");
                    const xmlParser = new XMLParser({
                        ignoreAttributes: false,
                        ...
                        textNodeName: "#text",
                        trimValues: true,
                        isArray: (name) => ["Country", "VehMatchedLoc", "VehMatchedLocs", "Code"].includes(name),
                        stopNodes: [],
 ...
 */
sourcesRouter.post("/sources/fetch-availability", requireAuth(), requireCompanyType("SOURCE"), async (req, res, next) => {
    try {
        const sourceId = req.user.companyId;
        const body = (req.body || {});
        const source = await prisma.company.findUnique({
        ...
        try {
            ...
            let raw = await fetchResponse.text();
            ...
            else if (typeof raw === "string" && (raw.trim().startsWith("{") && raw.includes("VehAvailRSCore"))) {
                try {
                    raw = JSON.parse(raw);
                }
                catch {
