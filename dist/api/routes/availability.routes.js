...
availabilityRouter.post("/submit", requireAuth(), requireCompanyStatus("ACTIVE"), async (req, res, next) => {
    ...
    try {
        ...
        if (Array.isArray(body.agreement_refs) && body.agreement_refs.length > 0) {
            ...
            if (ag) {
                const ok = await LocationsService.validateAgreementCoverage(ag.id, body.pickup_unlocode, body.dropoff_unlocode);
                if (!ok) {
    ...
    try {
        ...
        client.Poll({
            ...
            if (err) {
                ...
                const errorEntries = rawOffers.filter((o) => !!o?.error);
                const cleanOffers = rawOffers.filter((o) => !o?.error);
