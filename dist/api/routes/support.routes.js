 ...
 *       - bearerAuth: []
 */
supportRouter.get("/support/tickets", requireAuth(), async (req, res, next) => {
    try {
 ...
 *       - bearerAuth: []
 */
supportRouter.post("/support/tickets", requireAuth(), requireCompanyType("AGENT", "SOURCE"), async (req, res, next) => {
    try {
 ...
 *       - bearerAuth: []
 */
supportRouter.get("/support/tickets/:id", requireAuth(), async (req, res, next) => {
    try {
 ...
 *       - bearerAuth: []
 */
supportRouter.put("/support/tickets/:id", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        const ticketId = req.params.id;
        const schema = z.object({
 ...
 *       - bearerAuth: []
 */
supportRouter.get("/support/tickets/:id/messages", requireAuth(), async (req, res, next) => {
    try {
        const ticketId = req.params.id;
        const isAdmin = req.user.role === "ADMIN";
        const companyId = req.user.companyId;
 ...
 *       - bearerAuth: []
 */
supportRouter.post("/support/tickets/:id/messages/:messageId/read", requireAuth(), async (req, res, next) => {
    try {
        const ticketId = req.params.id;
        ...
        const shouldMarkRead = (isAdmin && (message.senderType === 'AGENT' || message.senderType === 'SOURCE')) ||
