...
locationsRouter.get("/locations/by-agreement/:agreementId", requireAuth(), async (req, res, next) => {
    try {
        ...
        client.ListCoverageByAgreement({ agreement_id: String(req.params.agreementId) }, metaFromReq(req), (err, resp) => (err ? next(err) : res.json(resp)));
    }
    catch (e) {
        next(e);
    }
...
locationsRouter.get("/agreements/:id/locations", requireAuth(), async (req, res, next) => {
    try {
...
locationsRouter.get("/locations", requireAuth(), async (req, res, next) => {
    try {
        const client = locationClient();
        client.ListUNLocodes({ query: String(req.query.query || ""), limit: Number(req.query.limit || 25), cursor: String(req.query.cursor || "") }, metaFromReq(req), (err, resp) => err ? next(err) : res.json(resp));
    }
    catch (e) {
        next(e);
    }
 ...
 */
locationsRouter.get("/coverage/source/:sourceId/branches", requireAuth(), requireCompanyType("ADMIN", "SOURCE", "AGENT"), async (req, res, next) => {
    try {
        const sourceId = String(req.params.sourceId || "").trim();
        if (!sourceId) {
            return res.status(400).json({ error: "BAD_REQUEST", message: "Source ID is required" });
 ...
 */
locationsRouter.post("/coverage/source/:sourceId/sync", requireAuth(), requireCompanyType("SOURCE"), async (req, res, next) => {
    try {
        ...
        const client = locationClient();
        client.SyncSourceCoverage({ source_id: sourceId }, metaFromReq(req), async (err, resp) => {
...
locationsRouter.post("/coverage/agreement/:agreementId/override", requireAuth(), async (req, res, next) => {
    try {
        ...
        const client = locationClient();
        client.UpsertAgreementOverride({ agreement_id: String(req.params.agreementId), unlocode: body.unlocode, allowed: body.allowed }, metaFromReq(req), (err, resp) => err ? next(err) : res.json(resp));
    }
    catch (e) {
        next(e);
    }
...
locationsRouter.delete("/coverage/agreement/:agreementId/override/:unlocode", requireAuth(), async (req, res, next) => {
    try {
        const client = locationClient();
        client.RemoveAgreementOverride({ agreement_id: String(req.params.agreementId), unlocode: String(req.params.unlocode) }, metaFromReq(req), (err, resp) => err ? next(err) : res.json(resp));
    }
    catch (e) {
        next(e);
    }
...
locationsRouter.post("/locations/request", requireAuth(), requireCompanyType("SOURCE"), async (req, res, next) => {
    try {
 ...
 *       - bearerAuth: []
 */
locationsRouter.get("/locations/requests", requireAuth(), requireCompanyType("SOURCE"), async (req, res, next) => {
    try {
        const sourceId = req.user.companyId;
 ...
 *       - bearerAuth: []
 */
locationsRouter.get("/locations/requests/:id", requireAuth(), requireCompanyType("SOURCE"), async (req, res, next) => {
    try {
        const { id } = req.params;
        const sourceId = req.user.companyId;
...
locationsRouter.patch("/admin/locations/requests/:id", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        const { id } = req.params;
 ...
 *       - bearerAuth: []
 */
locationsRouter.post("/admin/locations/requests/:id/approve", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        const { id } = req.params;
        const adminUserId = req.user.id;
 ...
 *       - bearerAuth: []
 */
locationsRouter.post("/admin/locations/requests/:id/reject", requireAuth(), requireRole("ADMIN"), async (req, res, next) => {
    try {
        const { id } = req.params;
        const adminUserId = req.user.id;
        const adminNotes = req.body.adminNotes;
