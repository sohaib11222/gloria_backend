 ...
 */
coverageRouter.get("/coverage/source/:sourceId", requireAuth(), async (req, res, next) => {
    try {
        ...
        const items = await buildCoverageListItems(sourceId, sourceLocations.map((sl) => ({
 ...
 *           type: string
 */
coverageRouter.post("/coverage/source/:sourceId/sync", requireAuth(), requireCompanyType("SOURCE"), async (req, res, next) => {
    try {
        const { sourceId } = req.params;
        const userCompanyId = req.user.companyId;
