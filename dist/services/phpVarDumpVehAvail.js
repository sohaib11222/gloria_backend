 ...
 */
function parsePhpString(str, start) {
 ...
 */
function parsePhpArray(str, start) {
 ...
 */
function parsePhpArrayContent(content) {
    ...
    if (allNumeric && entries.length > 0) {
        const maxIdx = Math.max(...entries.map(e => e.key));
        const arr = [];
        for (let j = 0; j <= maxIdx; j++)
            arr.push(result[j] !== undefined ? result[j] : undefined);
        return arr;
    }
    return result;
}
 ...
 */
export function convertPhpVarDumpToVehAvailRS(phpText) {
    ...
    if (firstArray === -1)
        throw new Error('Could not find root array');
    const openBrace = trimmed.indexOf('{', firstArray);
    if (openBrace === -1)
        throw new Error('Could not find root opening brace');
    const closeBrace = findMatchingBrace(trimmed, openBrace);
    if (closeBrace === -1)
        throw new Error('Could not find root closing brace');
 ...
 */
export function convertPhpVarDumpToObject(phpText) {
    if (!phpText || typeof phpText !== "string") {
        throw new Error("Invalid input: expected string");
    }
    const trimmed = phpText.trim();
