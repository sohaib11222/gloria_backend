...
export const AvailabilityStore = {
    ...
    async appendPartial(jobId, sourceId, items, timedOut = false) {
        ...
        for (const item of items) {
            await prisma.availabilityResult.create({
                data: { jobId, seq: seq++, sourceId, offerJson: item },
            });
        }
    },
    async markSourceDone(_jobId, _sourceId) {
    ...
    async getJobSince(jobId, sinceSeq, waitMs) {
        ...
        const readOnce = async () => {
            ...
            const status = job && job.status === "RUNNING" ? "IN_PROGRESS" : "COMPLETE";
            ...
            return {
                request_id: jobId,
                status,
                new_items: rows.map((r) => r.offerJson),
                last_seq: lastSeq,
    ...
    async purgeExpiredJobs(ttlSeconds = 600) {
