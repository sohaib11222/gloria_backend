 ...
 */
function normalizeAddressForClient(address) {
    ...
    return {
        ...
        async availability(criteria) {
            ...
            return await new Promise((res, rej) => client.GetAvailability(request, (e, r) => {
                ...
                const offers = (r.vehicles || []).map((v, index) => {
                    ...
                    if (!supplier_offer_ref) {
                        ...
                        const offerHash = Buffer.from(`${criteria.agreement_ref}-${v.vehicle_class || ""}-${v.make_model || ""}-${v.total_price || 0}-${index}`).toString('base64').substring(0, 8).replace(/[^A-Za-z0-9]/g, '');
                    ...
                    if (v.picture_url ?? v.pictureUrl)
                        base.picture_url = v.picture_url ?? v.pictureUrl;
                    if (v.door_count ?? v.doorCount)
                        base.door_count = String(v.door_count ?? v.doorCount);
                    if (v.baggage)
                        base.baggage = String(v.baggage);
                    if (v.vehicle_category ?? v.vehicleCategory)
                        base.vehicle_category = v.vehicle_category ?? v.vehicleCategory;
                    if (v.veh_id ?? v.vehId)
                        base.veh_id = v.veh_id ?? v.vehId;
                    if (v.ota_vehicle_json ?? v.otaVehicleJson) {
                        try {
                            const rich = JSON.parse(v.ota_vehicle_json ?? v.otaVehicleJson ?? "{}");
                            if (rich.veh_terms_included)
                                base.veh_terms_included = rich.veh_terms_included;
                            if (rich.veh_terms_not_included)
                                base.veh_terms_not_included = rich.veh_terms_not_included;
                            if (rich.vehicle_charges)
                                base.vehicle_charges = rich.vehicle_charges;
                            if (rich.total_charge)
                                base.total_charge = rich.total_charge;
                            if (rich.rate_distance)
                                base.rate_distance = rich.rate_distance;
                            if (rich.rate_qualifier)
                                base.rate_qualifier = rich.rate_qualifier;
                            if (rich.calculation)
                                base.calculation = rich.calculation;
                            if (rich.priced_equips)
                                base.priced_equips = rich.priced_equips;
                        }
                        catch (e) {
